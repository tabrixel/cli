...
- **WHEN** `tbxl rows update <id> --where 'Status=Open' --set 'Status=Done' --first --json` matches 3 rows
...
- **WHEN** `tbxl rows delete <id> --where 'Status=Done' --all --yes --json` matches 2 rows
...
- **WHEN** `tbxl rows update <id> --where 'Status=Nope' --set 'Status=Done' --all --json` matches no rows
