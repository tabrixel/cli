...
The CLI SHALL provide a `describe` command that reports, for each sheet of the document, the sheet name, its column names (parsed from the first row), and the number of records (rows below the header). The spreadsheet ID SHALL be taken from the positional `[SPREADSHEET_ID]` argument, falling back to `--spreadsheet-id`; if neither is set the command SHALL fail with `InvalidArguments`.

#### Scenario: Describe all sheets
- **WHEN** `tbxl describe <id>` is run without `--sheet` on a document with multiple sheets
...
- **WHEN** `tbxl describe <id> --sheet Tasks` is run and the sheet exists
